...
public class ImageMover extends JPanel implements MouseListener {
    ...
    public void mousePressed(MouseEvent e) {
        ...
        //check if the point where you click is inside image area
        Rectangle imageBounds = new Rectangle(x,y,img.getWidth(this), img.getHeight(this));
        
        if (imageBounds.contains(point)){
            //point is inside given image
            ++click;    //accumulate score
            if(click % 5 == 0 && speed > 150)
                speed = speed - 150;
            repaint();  //relocate the image's x/y coordinate
        }
    }
